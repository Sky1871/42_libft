...
1. Compilation
   `cd path/to/libft && make` <- run in the terminal
2. Usage
   `#include "libft.h"` <- include this header in the code
   `-lft -L path/to/libft.a -I path/to/libft.h` <- add these flags when compiling the code
